...
CREATE TABLE IF NOT EXISTS `oyun` (
  ...
  PRIMARY KEY (`id`),
  KEY `FK_oyun_oyun` (`yayinci_id`),
  KEY `FK_oyun_oyun_2` (`tur_id`),
  CONSTRAINT `FK_oyun_oyun` FOREIGN KEY (`yayinci_id`) REFERENCES `oyun` (`id`) ON DELETE NO ACTION ON UPDATE NO ACTION,
  CONSTRAINT `FK_oyun_oyun_2` FOREIGN KEY (`tur_id`) REFERENCES `oyun` (`id`) ON DELETE NO ACTION ON UPDATE NO ACTION
...
CREATE TABLE IF NOT EXISTS `yayinci` (
  `id` int(11) NOT NULL AUTO_INCREMENT,
  `kurtulus_tarih` date DEFAULT NULL,
  `parsonel` int(11) DEFAULT NULL,
  PRIMARY KEY (`id`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4;
